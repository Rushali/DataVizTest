...
export function BurndownPlot(crashes, {x, round = true, ...options} = {}) {
  const [start, end] = x.domain;
  const days = d3.utcDay.range(start, end);
  const burndown = crashes.flatMap((crash) =>
    Array.from(
      days.filter((d) => crash.created_at <= d && (!issue.closed_at || d < issue.closed_at)),
